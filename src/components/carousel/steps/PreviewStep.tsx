...
export default function PreviewStep({ config, hookTone, onRestart, onChangeHook, onSelectHook, onContentChange, onReload, initialImageStyle, initialContrastMode, initialMoodId, initialReelsMode, initialCitationFontSize, initialSlideBgColor, initialDarkBackground, initialLogoScale, initialArrowScale, initialHeadlineScale, initialBodyScale, initialIconScale, initialShowLuniaLifeWatermark, initialHookOverlays, initialShowSlideArrows, initialShowSlideNumbers, initialShowCitationBars, initialHookHeadlineWeight, initialHookImagesByWeight, initialEssayAccent, initialPillar, initialPaper, stylePreset = "default", carouselFormat = "standard", structure = null, initialSavedId = null, onSaved, initialVerification, onExit }: Props) {
  ...
  // Essay preset: yellow is the brand book's Signal Yellow; red is the essay-poster option.
  const [essayAccent, setEssayAccent] = useState<EssayAccent>(initialEssayAccent ?? "yellow");
  // Serial chrome. The number is stable per saved deck (and per topic before
  // the first save); the date is the day it is rendered.
  const essayNumber = essayNumberFrom(initialSavedId ?? config.topic ?? "");
  const essayDateText = essayDate();
  ...
  const [citationFontSize, setCitationFontSize] = useState(initialCitationFontSize ?? (isEssay ? 22 : isFreePress ? FP_TYPE.source : isEditorial ? 26 : 36));
  ...
  function updateSlideEmphasis(slideIndex: number, value: string | undefined) {
    ...
    void _drop;
    slides[slideIndex] = value === undefined ? rest : { ...rest, headlineEmphasis: value };
    onContentChange({ ...config, content: { ...content, slides } });
  }
  ...
  function captureLook(): CarouselLookSettings {
    return {
      ...
      showLuniaLifeWatermark, hookOverlays, showSlideArrows, showSlideNumbers, showCitationBars, hookHeadlineWeight, contentBgOverlayOpacity, essayAccent,
    };
  }
  ...
  function applyLook(s: CarouselLookSettings) {
    ...
    if (s.essayAccent !== undefined) setEssayAccent(s.essayAccent);
      ...
      return {
        ...
        body: (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {isEssay && (
              <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.06em", minWidth: 78 }}>Accent</span>
                {([{ value: "yellow", label: "Yellow", swatch: "#FFD800" }, { value: "red", label: "Red", swatch: "#D8321E" }] as const).map(({ value, label, swatch }) => (
                  <button key={value} onClick={() => setEssayAccent(value)} title={value === "yellow" ? "Signal Yellow, from the brand book" : "Essay-poster red"} style={{
                    display: "inline-flex", alignItems: "center", gap: 6, padding: "3px 8px", fontSize: 11, fontWeight: 700,
                    background: essayAccent === value ? "var(--text)" : "var(--surface)",
                    color: essayAccent === value ? "var(--bg)" : "var(--muted)",
                    border: "1px solid var(--border)", borderRadius: 5, cursor: "pointer", fontFamily: "inherit",
        ...
        body = (
          <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            <div>
              ...
              {isEssay && (
                <BoxedWordPicker headline={slide.headline} value={slide.headlineEmphasis} accent={ESSAY_COLORS.accent[essayAccent].fill} onChange={(w) => updateSlideEmphasis(slideIdx, w)} />
              )}
            </div>
            <div>
              {groupLabel("Size")}
        ...
        body: (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              ...
              {isEssay && (
                <BoxedWordPicker headline={takeaway.headline} value={takeaway.headlineEmphasis} accent={ESSAY_COLORS.accent[essayAccent].fill} onChange={(w) => updateTakeaway({ headlineEmphasis: w })} />
              )}
            </div>
              ...
              <>
                ...
                <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
                  {([
                    ...
                    { value: "high",     label: "Bold post", swatch: "linear-gradient(135deg, #01253F 60%, #FFD800 60%)" },
                  ] as const).map((opt) => {
                  ...
                  <div>
                    ...
                    {hook && (
                      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
                        ...
                        {isEssay && (
                          <BoxedWordPicker headline={hook.headline} value={hook.emphasis} accent={ESSAY_COLORS.accent[essayAccent].fill} onChange={(w) => updateHook(selectedHook, { emphasis: w })} />
                        )}
                      </div>
                    )}
