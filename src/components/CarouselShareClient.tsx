...
export default function CarouselShareClient({ carousel }: Props) {
  ...
  const {
    ...
    logoScale = 1, arrowScale = 1, darkBackground = false, slideBgColor, contentBgImages, contentBgOverlayOpacity, showLuniaLifeWatermark = false, hookOverlays,
    citationFontSize = 36,
    headlineScale = 1, bodyScale = 1, iconScale = 1,
    stylePreset = "default",
  ...
  const pillar = carousel.pillar ?? "Sleep";
  const paper = { grain: carousel.paperGrain ?? PAPER_DEFAULTS.billboard.grain, vignette: carousel.paperVignette ?? PAPER_DEFAULTS.billboard.vignette };
  const essayNumber = essayNumberFrom(carousel.id);
  const essayDateText = essayDate(carousel.savedAt);
  ...
  const previewNodes = [
    <HookSlide key={0} headline={hook.headline} subline={hook.subline} sourceNote={hook.sourceNote} topic={topic} scale={PREVIEW_SCALE} brandStyle={bs}
      ...
      stylePreset={stylePreset} showSlideArrows={showSlideArrows} showSlideNumbers={showSlideNumbers} showCitationBars={showCitationBars} headlineWeight={hookHeadlineWeight} headlineInImage={!!content.hookImageSpec} emphasis={hook.emphasis} essayAccent={essayAccent} pillar={pillar} paper={paper} essayNumber={essayNumber} essayDate={essayDateText} />,
    ...content.slides.map((s, i) => (
      <ContentSlideComponent key={i + 1} headline={s.headline} body={s.body} citation={s.citation} graphic={s.graphic} headlineEmphasis={s.headlineEmphasis} lineSpacing={s.lineSpacing} pillar={pillar} paper={paper} scale={PREVIEW_SCALE} brandStyle={bs} logoScale={logoScale} arrowScale={arrowScale} darkBackground={darkBackground} slideBgColor={slideBgColor} bgImageUrl={contentBgImages?.[i] ?? undefined} bgImageOverlayOpacity={contentBgOverlayOpacity} showLuniaLifeWatermark={showLuniaLifeWatermark} citationFontSize={citationFontSize} reels={reelsMode} headlineScale={headlineScale} bodyScale={bodyScale} iconScale={iconScale} essayAccent={essayAccent} essayNumber={essayNumber} essayDate={essayDateText} stylePreset={stylePreset} showSlideArrows={showSlideArrows} showSlideNumbers={showSlideNumbers} showCitationBars={showCitationBars} figure={s.figure} emphasis={s.emphasis} slideIndex={i} slideTotal={content.slides.length} />
    )),
