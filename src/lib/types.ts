...
export type SavedCarousel = {
  ...
  /** Decks saved before 2026-09-22 may still carry a `verification` record
   *  from the fact check. It is not read any more and not typed; the field is
   *  left on the stored object rather than stripped, so nothing is lost. */
  savedAt: string;
};
