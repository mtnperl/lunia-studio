...
- Caption Paragraph 4 (MANDATORY brand bridge — single sentence, ≤25 words):
  After the @lunia_life follow line, add one more sentence that ties THIS carousel's topic to ${LUNIA_BRAND.name} in a natural, on-voice way. Choose whichever bridge fits the topic best:
    ...
    (b) Ingredient bridge — connect the topic to one of the ${LUNIA_BRAND.product} ingredients at the amount printed on the label: magnesium bisglycinate 500mg (90mg elemental), L-theanine 300mg, or apigenin 50mg. Never call an amount a clinical dose. Pick the one most relevant to the topic's mechanism.
...
- All headlines uppercase.
- Caption: Instagram caption. ${includeSeoFooter ? "4" : "3"} paragraphs. The first two paragraphs are the post: open on the one specific detail from this deck the reader would not have guessed, said plainly, then say what it means for them, one idea per sentence, under 90 words together. Reuse the deck's own nouns, vary sentence length, no opening question, no rule-of-three lists, no "not X but Y" reversals. Close with "Comment [KEYWORD] below to get the full guide. Follow @lunia_life for more." No hashtags. No em dashes.${includeSeoFooter ? BRAND_BRIDGE_INSTRUCTION : ""}
...
10. Caption: 3 short paragraphs. The first two are the post: open on the one detail from the fact the reader would not have guessed, said plainly, then say what it means for them, one idea per sentence, under 80 words together. Reuse the fact's own nouns, vary sentence length, no rule-of-three lists, no "not X but Y" reversals. Close with a question or call to save the post. No hashtags. No em dashes.
