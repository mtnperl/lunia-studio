...
| 2026-07-22 | Staged block reveal permitted despite Motion section's "no decorative entrance animations" rule | Campaign editor's AI-suggested-layout feature reveals blocks with a short stagger as they land. Ruled functional, not decorative: it represents genuinely sequential AI output arriving, same category as the shimmer/skeleton loaders already sanctioned above for "work happening" — not an ornamental flourish on static content. Uses the existing 220ms ease-out panel timing, a fade only, no bounce/spring. |
| 2026-07-23 | Reverted app from Monday.com theme back to this Apple spec | `globals.css` had drifted to a Monday.com theme (blue `#0073ea` accent, Figtree, box-shadows) via the July editor redesign, diverging from this doc. Reverted all semantic tokens (both themes), fonts (Inter/Cormorant/Fira), and dropped shadows so the running app matches DESIGN.md again. This doc is the honored source of truth. |
| 2026-07-23 | Single `✨` marker permitted on AI-powered actions despite "no emoji in functional chrome" | All other emoji purged to SVG line-icons (`ui/icons.tsx`). The `✨` is kept ONLY on buttons that trigger an LLM call (Suggest layout, Suggest banner/promo, Regenerate subjects, Improve with Claude) as a deliberate "this costs an API call / this is AI" affordance. One intentional, scoped departure — not decorative, not applied to any non-AI control. |
| 2026-07-23 | Introduced `src/components/ui/` shared control system | `Button` (primary/secondary/ghost/danger + selected), `IconButton` (36×36, danger), `Label`, `Section`, `icons`. One app-adoptable vocabulary; classes live in `globals.css` (`.ui-btn*`, `.ui-icon-btn*`). Rest state uses `--text` so controls never read as disabled; `disabled` reserved for genuinely unavailable actions. Campaign editor adopted first; other screens follow. |
| 2026-08-22 | Built the display tier that this doc had specified since March | Cormorant Garamond was loaded in `globals.css` from day one and used in exactly ONE component (`CarouselLibraryView`, twice). The app's largest heading was 18px Inter, so no screen had a focal point and everything read as one flat grey field — the main reason it did not "feel expensive". Added `.display` + `.display-xl/lg/md` and a shared `ui/PageHeader`, applied to Home (greeting, italic accent, per the original spec), both builders and both libraries. |
| 2026-08-22 | Deleted the `--mon-*` palette outright | The July 23 revert removed the Monday.com theme but kept its categorical hues as "chip fills only", used in 26 places including the Home stat borders, action tiles and tone badges — 7 purple elements on the first screen you see, fighting the gold in dark mode. Chips are now ink-on-wash (`--chip-bg`/`--chip-fg`); colour is reserved for chips that report a STATE via `--success`/`--warning`/`--error`. UGC's pipeline pill became a progression (wash firming up, semantic colour only at the outcome stages) instead of six unrelated hues. |
| 2026-08-22 | `data-theme` on `<html>` is the only theme mechanism | `page.tsx` carried a second copy of the whole token table applied as INLINE styles, with a comment asking whoever touched it to "keep in lockstep with globals.css". Inline styles beat every stylesheet rule, so the copy always won and any token added to `globals.css` silently never switched themes. `applyThemeVars` now just sets the attribute; the CSS blocks that already existed are the single source. |
| 2026-08-22 | `.ui-btn--primary` label is `--bg`, not `#fff` | The accent is near-black in light and GOLD in dark, so a hardcoded white label sat at roughly 1.9:1 on gold everywhere the `ui/` system had been adopted. `--bg` is the ground the accent was chosen against, so it inverts correctly in both themes. Added `.ui-btn--lg` for flow-advancing CTAs, which the carousel steps had each hand-rolled slightly differently. |
| 2026-08-22 | Retired the retro terminal loaders | Generation showed a black CRT panel — scanlines, `◆ LUNIA.EXE`, a Courier block-bar and an `EXIT CODE: 1` error screen — inside a spec whose stated goal is "this feels expensive". Replaced with the shimmer this doc already sanctions for "work happening". The old carousel log also lied: its four lines were a fixed slice, so the same three steps always reported "OK" regardless of what the model was doing. |
| 2026-08-22 | Carousel step indicator is navigation | The four step labels were plain `div`s with no click handler, so from Preview there was no way back to Content — the only exits were "← Change hook" and "Start over" at the bottom of a 2,000px page. Now buttons gated on the furthest step reached, so anything you have already seen stays one click away. Work column widened 860 → 1240; the artwork had been getting less room than the controls beside it. |
| 2026-09-06 | Essay carousel preset: Anton + Caveat approved as a departure | The photoreal ivory-and-navy decks read as generated. The Essay preset adds the traces of a hand: paper grain, a condensed display face (Anton) with ONE word boxed in the accent, a script byline (Caveat), serial chrome (essay number, date, counter) and an engraving cover printed onto the paper. Accent is Signal Yellow by default with a red option per deck (`essayAccent`). Carousel-only; the app chrome stays on Inter/Cormorant/Fira. |
| 2026-09-07 | Essay slides may carry numbered list rows and one data graphic per deck | The essay body was a paragraph only. Added two readability shapes inside the same ink-and-accent vocabulary: list rows (Anton numeral in the accent on hairlines, the takeaway slide's own rows) for content that is a set, and at most one data graphic per deck (bars, split, stacked bar, stat, spectrum, timeline) drawn in ink with the accent, fitted and dropped rather than shrunk past legibility. No colour, no boxes, no icons; the cover engraving stays the deck's one picture. |
